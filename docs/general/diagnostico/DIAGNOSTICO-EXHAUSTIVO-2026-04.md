...
- **Mover a `docs/general/archive/`**: todos los `PASO-*-COMPLETADO`, `PORTAL-*-FASE-*-COMPLETADA`, `FIX-*`, `SOLUCION-*`, `MIGRACION-SHADCN-*`, `LISTO-PARA-PROBAR`, `LOGIN-ARREGLADO`, `REINICIAR-SERVIDOR`, `INSTRUCCIONES-REINICIO`. Total: ~50 docs.
- **Consolidar PROGRESO**: un único `docs/general/ESTADO.md` vivo.
- **Estructura deseable**:
  ```
  docs/
    README.md              ← índice
