...
export default async function SaasInvoicesPage({
  ...
  const [{ invoices, total, pageSize }, kpis] = await Promise.all([
    getSaasInvoices({
      status: (params.status as SaasInvoiceStatus) || undefined,
          ...
          <tbody>
            {invoices.map((inv) => (
              <tr key={inv.id} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <div className="font-medium">{inv.empresa?.name ?? '—'}</div>
                  <div className="font-mono text-[10px] text-gray-500">
                    {inv.empresa?.subdomain}
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-xs text-gray-600">
                  {inv.dueBy
                    ? format(inv.dueBy, 'dd MMM yyyy', { locale: es })
                    : '—'}
                </td>
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-1">
                    <a
                      href={`/api/billing/saas-invoice/${inv.id}/pdf`}
                      target="_blank"
                    ...
                    {(inv.status === 'ISSUED' || inv.status === 'OVERDUE') && (
                      <MarkPaidButton id={inv.id} kind="saas-invoice" />
                    )}
                  </div>
                </td>
              </tr>
            ...
            {invoices.length === 0 && (
              <tr>
                ...
                >
                  No hay facturas SaaS. Usa "Generar" en el dashboard billing.
                </td>
              </tr>
            )}
          </tbody>
          ...
          <div className="flex gap-2">
            {pageNum > 1 && (
              <Button variant="outline" size="sm" asChild>
                <Link
                  href={{
                    pathname: '/admin/billing/saas-invoices',
